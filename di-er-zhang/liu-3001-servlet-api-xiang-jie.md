...
1. init\(ServletConfig config\)
   ...
   主要是为了让servlet对象在处理客户请求前完成一些初始化工作。例如建立数据库的连接，获取配置信息等。对于每个Servlet实例，init()方法只能被调用一次。Servlet容器通过config参数向Servlet传递配置信息。可以通过config获取web应用程序的初始化参数，还可以获取ServletContext对象
   ```
...
7. Enumeration getParameterNames\(\)
...
9. String getProtocal\(\)

   ```
   返回请求使用的协议的名字和版本 例如: HTTP/1.1
   ```

10. RequestDispatcher getRequestDispatcher\(String path\)
...
12. int getServerPort\(\)

    ```
    返回请求发送到服务器的端口号
    ```

13. setCharacterEncoding\(String env\)
...
2. void setContentLength\(int len\)
...
1. 请求需要指定URL，浏览器根据URL生成HTTP请求并发送给服务器，请求的URL有一定的规范：
